...
public class UserGUI : MonoBehaviour {
	...
	private void OnGUI()
	{
		...
		if (action.is_win == 0)
		{
			...
			if (GUI.Button(new Rect(width * 6 - 50, hight * 2, 100, 30), "Go"))
			{
				action.Statelabel = "";
				action.Boat_Go();
			}
			if (GUI.Button(new Rect(width * 6 - 50, hight * 2+40, 100, 30), "Next"))
			{
				action.Statelabel = "";
				action.next();
			}
		...
		//StateLabel
		GUI.Box(new Rect(60, hight, 240, 250), "State");
		GUI.Label(new Rect(70, hight + 30, 220, 200), action.Statelabel);
		//button
